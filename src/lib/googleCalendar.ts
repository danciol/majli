...
export async function authorizeGoogleCalendar(employeeId: string): Promise<boolean> {
  ...
  return new Promise((resolve) => {
    const url = `https://accounts.google.com/o/oauth2/v2/auth?` +
      ...
      `state=${encodeURIComponent(employeeId)}&include_granted_scopes=true`;
...
export function buildCalendarEvent(appointment: { date: string; duration: number; clientName: string; serviceName?: string }): GoogleCalendarEvent {
  ...
  return {
    ...
    description: [`Klient: ${appointment.clientName}`, appointment.serviceName ? `Zabieg: ${appointment.serviceName}` : '', `Czas trwania: ${appointment.duration} min`].filter(Boolean).join('\n'),
